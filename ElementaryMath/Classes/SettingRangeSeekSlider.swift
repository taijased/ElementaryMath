...
@IBDesignable open class SettingRangeSeekSlider: UIControl {
    ...
    public convenience init(frame: CGRect = .zero, completion: ((SettingRangeSeekSlider) -> Void)? = nil) {
    ...
    open weak var delegate: SettingRangeSeekSliderDelegate?
    ...
    fileprivate func refresh() {
        if enableStep && step > 0.0 {
            ...
            if let previousStepMinValue = previousStepMinValue, previousStepMinValue != selectedMinValue {
                EngineTaptic.selection.feedback()
            }
...
private final class RangeSeekSliderLeftElement: UIAccessibilityElement {
    ...
    override func accessibilityIncrement() {
        guard let slider = accessibilityContainer as? SettingRangeSeekSlider else { return }
